...
calibration_source: Maxim product documentation + universal content discipline
owner: maxim
...
see_also:
  ...
  - .brand-foundation/personal.local/content-rules.md  (operator overlay, gitignored)
---

# Maxim Content Rules (Base Layer)

> Hard dos and don'ts that apply to every Maxim external-facing output.
> Per-startup positioning and operator overlays may add additional rules but
> cannot override the rules below. Pair with `voice-profile.md` (identity +
> patterns) and `ai-tells.md` (banned language).
>
...
> **Operator additions:** add operator-specific rules to `.brand-foundation/personal.local/content-rules.md` (gitignored).
...
- Use first person "I" only when framing a specific Maxim design decision. Prefer direct "you" for reader-facing content. Never "one." Never passive constructions.
- Attribute ideas to their origin when building on someone else's work. Framework citations name the originator (Fogg, Cialdini, Kahneman, Ryan and Deci, Thaler, etc.) inline on first mention.
...
- Write in third person about Maxim without a specific mechanism citation (avoid "Maxim revolutionizes..." — always name the mechanism).
...
1. Load Maxim base layer from `.brand-foundation/personal/` (voice-profile.md, ai-tells.md, content-rules.md)
2. Load operator overlay from `.brand-foundation/personal.local/` IF present (gitignored — operator's identity and writing-specific additions)
3. Detect active startup from `cwd` or `config/project-manifest.json`
4. Load matching `.brand-foundation/startups/{detected}/` overlays if present (positioning.md, audience.md, compliance-rules.md)
5. Apply precedence:
   - Maxim base defines the 3-voice architecture, signature patterns, em-dash prohibition, banned-phrase list, and structural rules (non-overridable)
   - Operator overlay extends identity and may add operator-specific banned phrases, preferred vocabulary, signature extensions
   - Startup overlay adds vertical-specific positioning, audience framing, and compliance rules
   - Startup compliance rules override operator rules for regulated content
6. Before output: run the voice-validation checklist from `voice-profile.md`
7. Before output: run the `ai-tells.md` banned-phrase scanner (base + operator overlay unioned)
8. Tag 🟢 HIGH only after both pass clean
