...
echo ""
info "Topology (ADR-013) — session-memory inheritance:"
info "  standalone = no inheritance (default)"
info "  parent     = this project reads child project states at session start"
info "  child      = this project reports its state to a parent at session end"
read -rp "$(echo -e "${BOLD}Topology kind${NC} (standalone / parent / child) [standalone]: ")" TOPO_KIND
TOPO_KIND=${TOPO_KIND:-standalone}
TOPO_CHILDREN="[]"
TOPO_PARENT="null"
if [[ "$TOPO_KIND" == "parent" ]]; then
  echo "  Enter child project paths (absolute), one per line. Empty line to finish:"
  CHILDREN_LIST=""
  while true; do
    read -rp "  Child path (or Enter to finish): " child_path
    [[ -z "$child_path" ]] && break
    CHILDREN_LIST+="\"${child_path}\","
  done
  CHILDREN_LIST="${CHILDREN_LIST%,}"
  [[ -n "$CHILDREN_LIST" ]] && TOPO_CHILDREN="[$CHILDREN_LIST]"
elif [[ "$TOPO_KIND" == "child" ]]; then
  read -rp "$(echo -e "  ${BOLD}Parent project absolute path${NC}: ")" parent_path
  TOPO_PARENT="\"${parent_path}\""
fi
