...
#   (agent-registry.json NOT copied — plugin-level only, served from install cache)
...
param(
  ...
  [Parameter(Mandatory=$false)]
  [ValidateSet("standalone","parent","child")]
...
function Build-ManifestJson {
  ...
  $topoKind     = if ($WizardData.TopoKind)     { $WizardData.TopoKind }     else { "standalone" }
  $topoChildren = if ($WizardData.TopoChildren) { $WizardData.TopoChildren } else { @() }
  $topoParent   = if ($WizardData.TopoParent)   { $WizardData.TopoParent }   else { $null }
  ...
  return @"
{
  ...
  "topology": {
    "kind": "$topoKind",
    "children": [$(if ($topoChildren.Count -gt 0) { '"' + ($topoChildren -join '", "') + '"' })],
    "parent": $(if ($topoParent) { '"' + $topoParent + '"' } else { "null" })
  },
...
function Invoke-SetupWizard {
  ...
  # ── Q10: Topology (ADR-013) ───────────────────────────────────────────────
  Write-Host ""
  Write-Host "  Topology (ADR-013) — parent/child session-memory inheritance:" -ForegroundColor White
  Write-Host "    standalone = no inheritance (default, safe for most projects)" -ForegroundColor DarkGray
  Write-Host "    parent     = this project reads child project states at session start" -ForegroundColor DarkGray
  Write-Host "    child      = this project reports its state to a parent at session end" -ForegroundColor DarkGray
  $topoKinds   = @("standalone","parent","child")
  $topoKind    = Read-WizardInput -Prompt "Topology kind" -Default "standalone" -ValidValues $topoKinds
  $topoChildren = @()
  $topoParent   = $null
  if ($topoKind -eq "parent") {
    Write-Host "  Enter child project paths (absolute), one per line. Empty line to finish:" -ForegroundColor Cyan
    while ($true) {
      Write-Host "  Child path (or Enter to finish) : " -NoNewline -ForegroundColor Cyan
      $raw = $Host.UI.ReadLine()
      if ($raw.Trim() -eq "") { break }
      $topoChildren += $raw.Trim()
    }
  } elseif ($topoKind -eq "child") {
    $topoParent = Read-WizardInput -Prompt "Parent project absolute path"
  }
  ...
  $wizardData = @{
    ...
    TopoKind       = $topoKind
    TopoChildren   = $topoChildren
    TopoParent     = $topoParent
  }
...
function Invoke-Bootstrap {
  ...
  # agent-registry.json is plugin-level only — NOT copied to user projects.
  # Claude reads it from the plugin install cache ($CLAUDE_PLUGIN_ROOT/config/agent-registry.json).
  # Absent in user projects is correct and expected — do not copy here.
  # Copying it would create stale divergent copies that trigger false "Top risk" warnings.
  ...
  New-File ([IO.Path]::Combine($PROJECT, ".mxm-skills", "skill-gaps.log")) @"
...
"@ ".mxm-skills/agents-handoff.md"
  ...
  New-File ([IO.Path]::Combine($PROJECT, ".mxm-skills", "review-queue.md")) @"
# Maxim Review Queue
...
"@
  ...
  $perProjectChecks = @(
    ...
    # agent-registry.json intentionally absent — plugin-level only, served from plugin cache
    ...
    @{ Path = [IO.Path]::Combine($PROJECT, ".mxm-skills", "skill-gaps.log");                          Label = ".mxm-skills/agents-skill-gaps.log" },
...
if ($WizardMode) {
  ...
  $cliData = @{
    ...
    UseGlobalClaude = $UseGlobalClaude
    TopoKind       = $Topology
    TopoChildren   = $Children
    TopoParent     = if ($Parent -ne "") { $Parent } else { $null }
  }
