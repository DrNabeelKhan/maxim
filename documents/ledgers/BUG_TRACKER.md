...
The ADR-021 router (`user-prompt-router.{sh,ps1}` + `config/routing-table.json`) classifies by whole-prompt keyword hits with an **array-order tie-break**, makes **no distinction between the operator's instruction and pasted/quoted text**, and treats **polysemous keywords** (`design`, `build`, `api`, `architecture`) as strong. Result: incidental words — especially inside pasted content — outvote intent, and 1-1 ties resolve by list position, not confidence. The mandatory `🧭` banner then renders a low-confidence guess as a certainty. **Struck 3× in one session (2026-07-03):** a "reply to this LinkedIn message" → CPO/design (via "stack design"), architecture Q&A → CTO/build (via "build"/"api"), a technical VAZIR spec → CMO/content. Mitigation (BUG-012): demote polysemous keywords to `weak_keywords` (require a co-signal), add missing intent verbs; **deferred** — a margin tie-break, quoted-content exclusion, and low-confidence banner suppression. **Standing guard:** the 5-case router smoke test (LinkedIn→CMO · UI-design→CPO · build→CTO · bare-design→passthrough · GDPR→CSO); any new *strong* keyword must be checked for polysemy before adding.
...
| **Status** | RESOLVED + HARDENED — pure-data fix (2026-07-03) + scorer hardening (2026-07-08), both committed **locally** in the batch (not yet tagged). |
