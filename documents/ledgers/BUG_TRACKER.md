...
| **Status** | RESOLVED (core symptom) — pure-data fix committed **locally** 2026-07-03; ships in the next batched release (not yet tagged). Broader hardening DEFERRED (below). |
...
| **Deferred hardening (next in batch)** | (i) **Margin tie-break** in `user-prompt-router.{sh,ps1}` — winner must beat runner-up by ≥1 strong hit else passthrough (kills order-dependence). (ii) **Quoted-content exclusion** — weight the operator's instruction over pasted quotes (root of all 3 misroutes). (iii) **Banner-confidence honesty** — on a 1-hit / no-margin match, fall silent or mark advisory instead of the confident `🧭` line → **ADR-021 default-on contract change, operator decision (flagged, not decided)**. |
