...
**Answer:** Maxim is a behavioral intelligence layer that runs on top of Claude. It adds **91 specialist agents** routed through 7 executive offices (24 dispatchable + 67 specialist catalog via mxm-catalog MCP per ADR-017) · **52 skill domains** · **78 peer-reviewed behavioral frameworks** dispatched per task · **14 compliance frameworks** enforced at the MCP layer · **9 MCPs (95 tools)** including NotebookLM research synthesis · **13 drift classes** scanning every session start · **structural framework citation** on every output (ADR-007) · **confidence tag** on every output (ADR-010). Every emission Maxim ships is grounded, audited, and traceable.
