...
> v1.3.8.1 refresh — supersedes the v1.3.2.3 submission snapshot (and earlier v6.4.4 / v1.3.2.1 drafts). Source-of-truth for capability counts: `documents/ledgers/AGENT_SKILL_INVENTORY.md`.
...
|---|---|---|
| Plugin manifest | `.claude-plugin/plugin.json` | v1.3.8.1 — declares 91 agents · 52 skills · 50 commands · 86 frameworks · 14 compliance · 9 MCPs (95 tools) · 90-day Trial of 14 packs default |
...
| Frameworks master | `documents/reference/FRAMEWORKS_MASTER.md` | 86 peer-reviewed behavioral frameworks with author/year citations + mechanism statements |
...
| Changelog | `CHANGELOG.md` | Release history from v1.0.0-rc.1 (2026-04-21) through v1.3.8.1 (2026-06-19). Pre-release-audit dispatched before every tag from v1.3.2 forward (disciplined ships — never self-claimed PASS). |
...
86 frameworks cataloged in `FRAMEWORKS_MASTER.md`. Each pack's SKILL.md cites a primary framework (Prospect Theory · Cognitive Load Theory · Signal Detection Theory · COM-B · Dual Process Theory · Fogg B=MAP). Frameworks are the moat — not the count of agents.
...
The Anthropic Claude Code plugin marketplace ingestion process **is not codified in this document** because the canonical process lives at `https://docs.claude.com/claude-code/plugins/marketplace` and may change. As of v1.3.8.1 submission preparation (2026-06-19), three paths are likely:
...
| Display name | Maxim |
| Version | `1.3.8.1` |
| Description | The behavioral intelligence layer for Claude. 91 specialist agents across 7 executive offices, 52 skill domains, 50 slash commands, 86 peer-reviewed behavioral frameworks, 14 compliance frameworks, 9 MCPs (95 tools) including NotebookLM research synthesis, license-gated MCP middleware, 13-class proactive drift detection. 90-day Trial of all 14 packs default at install. BSL 1.1 licensed. |
...
- [x] Plugin manifest version, marketplace.json (outer + plugin entry), README badge, AND this submission doc all agree on `1.3.8.1` (Bucket 1 version-consistency check per the v1.3.2.1 amendment that catches outer catalog metadata.version drift)
...
- [x] `v1.3.8.1` tag pushed to `origin/main` (SHA in git tag annotation); `main` == `origin/main`
...
## Known limitations at v1.3.8.1 submission
...
4. **ADR-009 amendment pending.** ADR-009 defines L2 as "subscription tiers" (Solo/Pro/Professional/Team); v1.3.0 wizard ships L2 as "vertical bundle packs" (founder-os/growth-stack/pro-os/agency-all). PACKS.md reconciles this in operator-facing docs; the ADR text amendment is not yet written (as of v1.3.8.1).

5. **ADR-017 amendment pending.** ADR-017 text still says "~19 dispatchable subagent files" (§Layer-1); actual count is 24 (per `plugin.json` agents[] array length). Reality is correct in all operator-facing surfaces; the ADR text amendment is not yet written (as of v1.3.8.1).
