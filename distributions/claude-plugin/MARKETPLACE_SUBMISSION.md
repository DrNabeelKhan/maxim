...
> v1.3.2.1 refresh — supersedes the pre-launch v6.4.4 draft. Source-of-truth for capability counts: `documents/ledgers/AGENT_SKILL_INVENTORY.md`.

**Target:** Anthropic Official Claude Code Plugin Marketplace
**Plugin namespace:** `maxim`
**Submission version:** 1.3.2.1
**Tag:** `v1.3.2.1` (commit `ac4a7fe7b3f28f7d4ed9c93759138eab8aee89a0`)
...
Live since 2026-04-21 (v1.0.0 launch). Currently at v1.3.2.1. Used by 21+ projects across the iSystematic portfolio.
...
|---|---|---|
| Plugin manifest | `.claude-plugin/plugin.json` | v1.3.2.1 — declares 91 agents · 36 skills · 48 commands · 74 frameworks · 14 compliance · 9 MCPs (95 tools) · 90-day Trial of 14 packs default |
| Marketplace manifest | `.claude-plugin/marketplace.json` | v1.3.2.1 — pack catalog (Maxim base plugin + 14 paid packs across L1 · L2 · L3 tiers) |
| README | `README.md` (root) | v1.3.2.1 with 9-MCP/95-tool table, 6 use cases, ADR index (15 public · 4 confidential) |
| LICENSE | `LICENSE` (root) | BSL 1.1 — converts to Apache 2.0 after 4 years per ADR-005 |
| Version | `1.3.2.1` consistent across plugin.json + marketplace.json + README badge | Verified by Cycle 2 grep of v1.3.2.1 pre-release-audit |
...
| Changelog | `CHANGELOG.md` | Release history from v1.0.0-rc.1 (2026-04-21) through v1.3.2.1 (2026-05-20). Pre-release-audit dispatch documented for v1.3.2 + v1.3.2.1. |
...
The Anthropic Claude Code plugin marketplace ingestion process **is not codified in this document** because the canonical process lives at `https://docs.claude.com/claude-code/plugins/marketplace` and may change. As of v1.3.2.1 submission preparation (2026-05-20), three paths are likely:
...
| Release tag | `v1.3.2.1` |
| Commit SHA | `ac4a7fe7b3f28f7d4ed9c93759138eab8aee89a0` |
...
- [x] Plugin manifest version, marketplace.json version, README badge all agree on `1.3.2.1` (verified Cycle 2 of v1.3.2.1 pre-release-audit)
...
- [x] `git log --oneline` shows `v1.3.2.1` tag at commit `ac4a7fe7`; tag pushed to `origin/main`
...
## Known limitations at v1.3.2.1 submission

1. **BUG-008 OPEN — Windows Git Bash registry-update silent failure.** `mxm-self-update.sh` Python heredoc receives MSYS-style `/c/Users/...` path that Python on Windows cannot resolve. Registry SHA never updates on Windows; operators on Mac/Linux/WSL unaffected. Manual remediation: edit `~/.claude/plugins/installed_plugins.json` to set correct `gitCommitSha` after self-update. Fix scheduled for v1.3.3 (use `pathlib.Path.home()` inside heredoc).

2. **First-restart cold-warm on Windows.** After self-update, the first Claude Code restart takes 5-10 minutes while 9 MCPs cold-spawn concurrently and Windows Defender scans node_modules. Subsequent restarts are normal speed. Warning is now in `mxm-self-update.{sh,ps1}` completion banners (v1.3.2.1).
